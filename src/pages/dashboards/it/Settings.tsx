  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data, error } = await supabase
        .from("staff_profiles")
        .select("id, full_name, username, email, phone_number, role, status, availability_status")
        .eq("id", user.id)
        .maybeSingle();
        ...
        {loading ? (
          ...
          <div className="space-y-5">
            ...
            <div className="grid sm:grid-cols-3 gap-4 pt-2">
              ...
              <div className="space-y-1.5">
                <Label>Status</Label>
                <div className="flex h-10 items-center"><Badge variant="outline">{data.status ?? "—"}</Badge></div>
              </div>
              <div className="space-y-1.5">
                <Label>Availability</Label>
                <div className="flex h-10 items-center"><Badge variant="outline">{data.availability_status ?? "—"}</Badge></div>
              </div>
            </div>
